<!DOCTYPE html>
<html lang="es">
<head>
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
  <title>Registros</title>
  <link rel="stylesheet" href="css/plantilla_captura.css" media="all" />
  <style media="screen">
   @page { size: 20cm 25cm landscape; }
  </style>
</head>
<body>
  <header class="clearfix">
    <div id="logo">
      <img src="img/logopetc2.jpg"  width="800"  height="70"/>



    </div>



    <h1>No. de Registros: </h1>
    <div id="project" >
      <div><span> Programa</span> PROGRAMA ESCUELAS DE TIEMPO COMPLETO</div>
      <div><span>Área: </span> Nomina y Sistemas</div>
      <div><span>Email: </span> NOMINA.ETC@GMAIL.COM</div>
      <div><span>Tel: </span>9220666 EXT: 5403-5405</div>
    </div>

    <div id="project2" align="right" >

    </div>
  </header>

  <main>

    <table name="table_producto" id="table_producto" border="0" cellspacing="0" cellpadding="0">
      <thead>
        <tr>
          <th>Nombre </th>
          <th>RFC</th>
          <th>Telefono</th>
          <th>Email</th>
          <th>Clave</th>
          <th>Sostenimiento</th>
          <th>Categoria</th>
          <th>Estado</th>
          <th>Fecha de registro</th>
          <th>Captura</th>

        </tr>
      </thead>
      <tbody>
        @foreach($personal as $personal)
        <tr>
          <td>{{$personal->nombre}} </td>
          <td>{{$personal->rfc}} </td>
          <td>{{$personal->telefono}}</td>
          <td>{{$personal->email}}</td>
          <td>{{$personal->clave}}</td>
          <td>{{$personal->sostenimiento}}</td>
          <td>{{$personal->categoria}}</td>
          <td>{{$personal->estado}}</td>
          <td>{{$personal->created_at}}</td>
          <td>{{$personal->captura}}</td>



        </tr>
        @endforeach
      </tbody>
      <tfoot>

      </tfoot>
    </table>
        <div><?php echo DNS2D::getBarcodeHTML('{$entrada->capturas}', "QRCODE",3,3);?></div>
<br/>
        <div><?php echo DNS1D::getBarcodeHTML('{$entrada->capturas', "C128",1,40);?></div>
  </body>
  </html>
